...
# 3.
출력 신호를 알아내기 위해 우선, 입력 신호의 z-transform된 형태를 구합니다.
$$\begin{align*}
E^{d}(z)&= \sum\limits^{\infty}_{k=0}2\sin\left(\frac{k}{2}\right)z^{-k}\\
&= \frac{z\sin\left(\frac{1}{2}\right)}{z^{2}-2z\cos\left(\frac{1}{2}\right)+1}\end{align*}$$
$U^{d}(z)=E^{d}(z)D^{d}(z)$를 구합니다.
$$\begin{align*}
U^{d}(z)&= \frac{z\sin\left(\frac{1}{2}\right)}{z^{2}-2z\cos\left(\frac{1}{2}\right)+1}\cdot \frac{2z+2}{z-1}\\
&= ??
\end{align*}$$

$U^{d}(z)$를 inverse z-transform 하여 $u^{d}(k)$를 구합니다.
$$??$$

답은.. 뭐지..
$$7.83279 (2 - 200000^{-n} ((175517 - 95884.2 i)^{n} + (175517 + 95884.2 i)^{n}))$$